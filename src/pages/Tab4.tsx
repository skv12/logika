...
const Tab4: React.FC = () => {
  ...
  function SStatus(data) {
    ...
    axios
      ...
      .then((data) => {
        if (
          dist.Состояние === "Доставлено" &&
          params.params.Тип === "Доставлено"
        ) {
          ...
          <>
            ...
            >
              <IonGrid>
                <IonRow>
                  <IonCol size="4">
                    <IonRow>{info[i].Номер}</IonRow>
                    <IonRow>{info[i].Дата}</IonRow>
                  </IonCol>
  ...
  function Distant(): JSX.Element {
    ...
    if (dist === undefined) elem = <Dist info={Store.getState().dist} />;
    else elem = <DCard info={dist} />;
    return elem;
  }
